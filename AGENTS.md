...
- Fluxo de publicação de release:
  ...
  5. Atualizar documentação em `docs/`. Atualizar a seção O Que Há de Novo (`whats-new.rst`) com as novidades da release. Ao criar nova série de versão (ex: 1.8.x): copiar pastas da versão anterior (`docs/pt-br/<versao-anterior>/` e `docs/en/<versao-anterior>/`), atualizar `conf.py` (`release`, `doc_path`), adicionar alvos no `docs/Makefile`, atualizar seletor/função de versão em `docs/_templates/layout.html` e ajustar redirecionador em `docs/index.html`.
...
- cada release nova: atualizar whats-new.rst em pt-br e en com resumo das mudancas da versao.
